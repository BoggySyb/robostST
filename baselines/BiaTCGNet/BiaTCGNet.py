from baselines.BiaTCGNet.BiaTCGNet_layer import *
import torch.nn as nn
import torch
from torch.nn.utils import weight_norm

class Model(nn.Module):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, kernel_set, device, 
        predefined_A=None, static_feat=None, dropout=0.3, subgraph_size=5, node_dim=40, dilation_exponential=1, 
        conv_channels=32, residual_channels=32, skip_channels=64, end_channels=128, seq_length=12, in_dim=2, out_len=12, out_dim=1,
        ...
        for i in range(1):
            ...
            for j in range(1,layers+1):
                ...
                if self.gcn_true:
                    self.gconv1.append(mixprop(conv_channels, residual_channels, gcn_depth, dropout, propalpha, dilationsize[j-1],num_nodes,self.seq_length,out_len,device))
                    self.gconv2.append(mixprop(conv_channels, residual_channels, gcn_depth, dropout, propalpha, dilationsize[j-1],num_nodes,self.seq_length,out_len,device))
    ...
    def forward(self, input, mask, k, idx=None): #tx,id
        ...
        k: int topk
        """
        input = input.transpose(1,3) # B,D,N,W
        mask = mask.transpose(1,3).float() # B,D,N,W
        ...
        x = self.start_conv(input) # B,D_new,N,W
